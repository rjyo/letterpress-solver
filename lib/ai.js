...
var weightNearby = function(boardStr, board, i, results) {
  ...
  var checkNearDeepRed = (board[i] == -2);
  var checkNearDeepBlue = (board[i] == 2);
  ...
  for (var i = 0; i < nearby.length; i++) {
    ...
    if (checkNearDeepRed) {
      results[index] += DEF.WEIGHT_NEAR_DEEP_RED;
    }

    if (checkNearDeepBlue && board[index] == 2) {
      results[index] += DEF.WEIGHT_DEEP_BLUE_WITH_DEEP_BLUE;
    }
