...
var arrayWithoutDup = function(words) {
  var results = [];
  for (var i = 0; i < words.length; i++) {
    var fail = false;
    for (var j = 0; j < results.length; j++) {
      if (isSameArray(results[j], words[i])) {
        fail = true;
        break;
      }
    }
    if (!fail) results.push(words[i]);
  }
  return results;
}
...
var isSameArray = function(a1, a2) {
  ...
  for (var i = 0; i < a1.length; i++) {
    if (!~a2.indexOf(a1[i])) return false;
  }
  return true;
}
...
var wordPosition = function(word, board) {
  ...
  return arrayWithoutDup(results);
}
...
exports.arrayWithoutDup = arrayWithoutDup;
